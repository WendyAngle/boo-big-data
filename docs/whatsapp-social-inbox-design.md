...
本节记录对 v2 主体设计的审查结论。**"已修正"项已内联更新到上文对应章节**；
"待业务确认"项列出以便后续讨论。
...
| # | 问题 | 修正位置 |
|---|---|---|
| 1 | 顶部工具栏 tab 与 Phase 1 描述不一致（`未分配` 既是 tab 又是筛选） | §7.1 只在状态筛选保留 `未分配` |
| 2 | Tab 未读数含义不清（易被池内未读淹没） | §7.1 改为"分配给我的未读" |
| 3 | 抢单并发未处理，两人同时点"我来跟"会双分配 | §7.5 加乐观锁 SQL |
| 4 | SLA 起算规则缺失，未分配阶段可能挂在无辜员工头上 | §四·SLA 拆分为"池 SLA / 个人 SLA" |
| 5 | 通知策略遗漏（谁在什么时候被叫醒） | §四 新增"通知策略" |
| 6 | `snoozed` 状态无唤醒机制 | §四 新增"Snooze 唤醒" |
| 7 | 派单缺易用性辅助：老客户回来时找不到原跟进人 | §7.5 加"上次跟进人优先"候选排序 |
| 8 | 共享 WA 号转派后客户不知换人 | §7.5 加"发送切换招呼语"选项 |
| 9 | 分配浮层完全禁止跨分组，实际救场场景无法处理 | §7.5 允许管理员显式跨组转派 |
| 10 | AI 草稿"必须点发送"但未强制编辑/元数据留痕 | §六·AI 加 `ai_assisted=true` 元数据 |

### 建模层面的重要澄清（不改表结构，仅明确语义）

- **`contacts.target_type` 的语义**：表示该联系人**主要业务身份**（是"公司主体"还是"自然人 KP"），决定其新会话默认落入哪个池；不代表现实世界"不能既是公司又是人"。极少数需要切换的情况由管理员手动改。
- **企业池 vs 人物池的实际分布**：WA/Telegram/Facebook 账号本质都是自然人载体，因此**人物池会占绝大多数**；企业池主要承接企业邮箱、企业公众号、企业 WA 商号等对公通道。这符合业务预期，不是失衡。
- **`channel_account_id` 对 email 渠道**：沿用 `src/lib/mailboxes.ts` 现有邮箱账号，不重复建模。

### 待业务确认（不阻塞 Phase 1）

1. **组长是否默认能看到全组会话**（含未分配 + 他人已分配）？建议：是，只读默认，操作需二次确认；
2. **管理员批量派单是否支持"平均分配"一键**？虽然整体策略是人工，但组长手工点一百个太累；建议提供"平均分给勾选成员"的批量操作按钮，仍属人工触发；
3. **snooze 上限**：是否限制最长 30 天，避免会话被无限期挂起变成僵尸；
4. **意向评分是否影响列表默认排序**：默认按 `last_message_at`，是否加"高意向优先"toggle？

---

_审查更新：2026-07-08 · v2.1_